...
## Epic: 전국 다중 창고 기반 물류망 확장 (P4a 완료)
...
배송지 기반 라우팅에 필요한 데이터가 없다 — 센터 좌표, 주문 배송지 좌표,
권역 → 센터 우선순위. `shippingAddress` 는 문자열뿐이라 지오코딩 없이는 주소 파싱이
되고, 그러면 전국 물류망 설계가 아니라 주소 처리 코드에 시간을 쓰게 된다.
만들려면 `Center` 좌표 컬럼과 권역 매핑 테이블이 먼저다.
